...
(function() {
	...
	if(jQuery('#ucpm-advanced-map').length <= 0) {
		return false;
	}
	 ...
	 */
	function initMap() {
		propertiesData = jQuery.parseJSON( jQuery('.ucpm-google-map').attr('data-properties-data') );
		jQuery('.ucpm-google-map').removeAttr('data-properties-data');
		var map_settings = propertiesData.map_settings;
		...
		map = new google.maps.Map(document.getElementById("ucpm-advanced-map"), mapOptions);
		...
		var properties = propertiesData.properties;
	...
	function initMarkers(map, markerData) {
			...
			infoboxOptions = {
				content: '<div class="map-marker-wrapper">' +
							'<div class="map-marker-container">' +
								'<div class="close-infobox"><i class="ucpm-icon-close"></i></div>' +
		...
		if(propertiesData.map_settings.fit == 'true') {
	...
	function map_controls() {
		...
		// Zoom In
		google.maps.event.addDomListener(document.getElementById('ucpm-zoom-in'), 'click', function (e) {
			e.preventDefault();
		...
		// Zoom Out
		google.maps.event.addDomListener(document.getElementById('ucpm-zoom-out'), 'click', function (e) {
			e.preventDefault();
			var currentZoom = map.getZoom();
			currentZoom--;
			if (currentZoom > 19) {
		...
		// Map Type: Roadmap
		google.maps.event.addDomListener(document.getElementById('ucpm-map-type-roadmap'), 'click', function (e) {
			e.preventDefault();
		...
		// Map Type: Satellite
		google.maps.event.addDomListener(document.getElementById('ucpm-map-type-satellite'), 'click', function (e) {
			e.preventDefault();
		...
		// Map Type: Hybrid
		google.maps.event.addDomListener(document.getElementById('ucpm-map-type-hybrid'), 'click', function (e) {
			e.preventDefault();
		...
		// Map Type: Terrain
		google.maps.event.addDomListener(document.getElementById('ucpm-map-type-terrain'), 'click', function (e) {
			e.preventDefault();
			map.setMapTypeId(google.maps.MapTypeId.TERRAIN);
			return false;
		});

		if( propertiesData.map_settings.search == 'true' ) {
			jQuery("#ucpm-map-address").geocomplete();
			document.getElementById('ucpm-map-submit').addEventListener('click', function() {
				geocodeAddress( geocoder, map );
			});

			function geocodeAddress(geocoder, resultsMap) {
				var search_zoom = parseInt(propertiesData.map_settings.search_zoom);
				var address = document.getElementById('ucpm-map-address').value;
				geocoder.geocode({'address': address}, function(results, status) {
				if (status === 'OK') {
		...
		jQuery('#ucpm-current-location').click(function (e) {
			e.preventDefault();
			...
			if (!jQuery('#ucpm-current-location').hasClass('draw')) {
				// Create Loading Element
				var loading = document.createElement('div');
				loading.setAttribute('class', 'ucpm-loader-container');
				loading.innerHTML = '<div class="svg-loader"></div>';
				document.getElementById('ucpm-advanced-map').appendChild(loading);
			}
			...
			if (navigator.geolocation) {
				...
				navigator.geolocation.getCurrentPosition(function (current) {
					...
					jQuery('#ucpm-current-location').addClass('draw');

					// Toggle Crrent Location Icon & Circle
					if (jQuery('#ucpm-current-location').hasClass('active')) {
