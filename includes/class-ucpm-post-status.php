...
class ucpm_Post_Status {
	...
	function archive_post_status(){
		register_post_status( 'archive', array(
			...
			'label_count'				=> _n_noop( 'Archive <span class="count">(%s)</span>', 'Archived <span class="count">(%s)</span>', 'ucpm' ),
		) );
	}
		...
		if( $post->post_type !== 'listing' )
			...
			<script>
				jQuery( document ).ready( function( $ ) {
					$( '#post_status' ).append( '<option value="archive"><?php esc_html_e( 'Archived', 'ucpm' ) ?></option>' );
				} );
			</script>
			<?php
		}
	...
	function edit_screen_js() {
		...
		<script>
			jQuery( document ).ready( function( $ ) {
				...
				$( 'select[name="_status"]' ).append( '<option value="archive"><?php esc_html_e( 'Archived', 'ucpm' ) ?></option>' );
	...
	function display_post_states( $post_states, $post ) {
		...
		if ( $post->post_type !== 'listing' || 'archive' !== $post->post_status || 'archive' === get_query_var( 'post_status' ) ) {
			return $post_states;
		}
